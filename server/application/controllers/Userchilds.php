  ...
  public function reg()
  {
    Mvv\mvvLogin::check(self::role_name, function ($userinfor) {
      try {
        ...
        // 添加亲子关系
        $user_child_uid =  Model\xonUserChilds::add($user_id, $child_id, $relation_id);
        Model\xonUserGroup::add($user_id, Model\x5on::GROUP_STUDENT_PARENT);

        // 刷新孩子列表
        $result = Model\xovUserChilds::getByUid($user_child_uid);
  ...
  public function student() {
    Mvv\mvvLogin::check(self::role_name, function ($userinfor) {
      try {
        ...
        $this->json(['code' => 0, 'data' => compact('child', 'gradestuds')]);
      } catch (Exception $e) {
        $this->json(['code' => 1, 'data' => $e->getMessage()]);
      }
    }, function ($error) {
